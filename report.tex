...
\begin{description}
\item[LLM seuls] Les modèles flagship (400B+) retournent 30--40 centrales en requête directe, contre plus de 110 dans l'inventaire de référence. Une approche conversationnelle avec relances permet d'atteindre 80--90 centrales, confirmant que les modèles ne révèlent qu'un tiers de leur connaissance à la première réponse.
\item[RAG] L'intégration de documents sources améliore significativement les résultats. Avec Claude~3.5~Sonnet, RAG et relances, on atteint 61 correspondances exactes et 12 approximatives. Les deux techniques (relance et RAG) sont complémentaires.
...
\textbf{Ce qui fonctionne}~: L'approche RAG avec relances conversationnelles produit déjà des résultats utiles. La conversion PDF vers Markdown, bien qu'imparfaite, est opérationnelle. Les graphes globaux comme OpenStreetMap fournissent des données complémentaires de qualité.

\textbf{Ce qui est nécessaire}~: Un graphe de connaissances local aligné sur une ontologie (OEO) pour assurer la traçabilité. Des agents spécialisés (curateur, analyste, rédacteur) pour décomposer et automatiser le processus. Une architecture en quatre niveaux (document store, base validée, couche vectorielle, graphe structuré) pour combiner rigueur et flexibilité.
...
Les résultats de ce rapport montrent que l'automatisation complète de la production statistique n'est pas encore à portée, mais qu'une approche hybride -- combinant RAG, graphes de connaissances et agents spécialisés, avec supervision humaine -- est réaliste. Le cas du Vietnam constitue un premier benchmark~; les méthodes proposées sont transposables à d'autres pays et à d'autres secteurs nécessitant un suivi rigoureux des transformations socio-techniques.
