...
## Manuscript numbers go through generated macros (ticket 0531)

Every empirical number quoted in `slides/manuscript/main.tex` comes from a
`\newcommand` in `report/inputs/generated/macros_manuscript.tex` — a committed
artifact that `experiments/render.mk` consolidates from the per-script
fragments listed in `MANUSCRIPT_MACRO_FRAGMENTS`. To add a value: extend the
emitter script that owns the fragment, rebuild, commit the regenerated
fragment and the consolidated file together. Never hand-type a number into
the prose or into the macros file.
