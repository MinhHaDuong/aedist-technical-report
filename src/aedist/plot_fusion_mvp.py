...
def write_macros(results: list[RegimeResult], output: Path) -> None:
    ...
    def _frac(v: float) -> str:
        # Unitless 3-d.p. fraction variant (ticket 0531) — for prose that
        # quotes scores on the 0–1 scale rather than as percentages.
        return f"{v:.3f}"

    # LaTeX control words are letters-only: digits in a \newcommand name make
    # TeX stop reading the name early and reject the definition, so the regime
    # tag is spelled out ("E1" -> "EOne", "E2-1D" -> "ETwoOneD").
    regime_tags = {"E1": "EOne", "E2-1D": "ETwoOneD"}
    ...
    for res in results:
        ...
        lines += [
            f"% Regime: {res.regime}",
            rf"\newcommand{{\FusionN{reg}Runs}}{{{res.n_runs}}}",
            rf"\newcommand{{\FusionN{reg}Models}}{{{res.n_models}}}",
            rf"\newcommand{{\FusionBest{reg}F}}{{{_pct(res.best_single_f1)}\%}}",
            rf"\newcommand{{\FusionBest{reg}Recall}}{{{_pct(res.best_single_recall)}\%}}",
            rf"\newcommand{{\FusionBest{reg}Prec}}{{{_pct(res.best_single_precision)}\%}}",
            rf"\newcommand{{\FusionBest{reg}FFrac}}{{{_frac(res.best_single_f1)}}}",
            rf"\newcommand{{\FusionBest{reg}RecallFrac}}{{{_frac(res.best_single_recall)}}}",
            rf"\newcommand{{\FusionBest{reg}PrecFrac}}{{{_frac(res.best_single_precision)}}}",
            rf"\newcommand{{\FusionBest{reg}TP}}{{{best_tp_int}}}",
            rf"\newcommand{{\FusionBest{reg}FP}}{{{best_fp_int}}}",
            rf"\newcommand{{\FusionUnion{reg}F}}{{{_pct(res.union_f1)}\%}}",
            rf"\newcommand{{\FusionUnion{reg}Recall}}{{{_pct(res.union_recall)}\%}}",
            rf"\newcommand{{\FusionUnion{reg}Prec}}{{{_pct(res.union_precision)}\%}}",
            rf"\newcommand{{\FusionUnion{reg}FFrac}}{{{_frac(res.union_f1)}}}",
            rf"\newcommand{{\FusionUnion{reg}RecallFrac}}{{{_frac(res.union_recall)}}}",
            rf"\newcommand{{\FusionUnion{reg}PrecFrac}}{{{_frac(res.union_precision)}}}",
            rf"\newcommand{{\FusionUnion{reg}TP}}{{{res.union_tp}}}",
            rf"\newcommand{{\FusionUnion{reg}FP}}{{{res.union_fp}}}",
            rf"\newcommand{{\FusionRTwo{reg}F}}{{{_pct(res.r2_f1)}\%}}",
            rf"\newcommand{{\FusionRTwo{reg}Recall}}{{{_pct(res.r2_recall)}\%}}",
            rf"\newcommand{{\FusionRTwo{reg}Prec}}{{{_pct(res.r2_precision)}\%}}",
            rf"\newcommand{{\FusionRTwo{reg}FFrac}}{{{_frac(res.r2_f1)}}}",
            rf"\newcommand{{\FusionRTwo{reg}RecallFrac}}{{{_frac(res.r2_recall)}}}",
            rf"\newcommand{{\FusionRTwo{reg}PrecFrac}}{{{_frac(res.r2_precision)}}}",
            rf"\newcommand{{\FusionRTwo{reg}TP}}{{{res.r2_tp}}}",
            rf"\newcommand{{\FusionRTwo{reg}FP}}{{{res.r2_fp}}}",
            "",
        ]
