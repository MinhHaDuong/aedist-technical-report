...
def render_recognition_matrix(
    ...
    lang: str = "en",
    suptitle: str = "Which Vietnamese thermal assets does each model recognize?",
    macros_prefix: str = "ExpOneMatrix",
) -> None:
    """Render a recognition matrix PDF (and optional macros) from a RecognitionData object.

    Shared renderer used by both ``plot_exp1_matrix`` (Exp1 records) and
    ``plot_exp2_matrix`` (Exp2 mart arms).  The caller is responsible for loading
    and filtering the data; this function handles only the matplotlib layout.

    Args:
        data: Pre-loaded and pre-filtered :class:`RecognitionData`.
        output: Output PDF path.
        output_macros: Optional ``.tex`` file for plant/run/FP count macros.
        fp_top_n: Number of top false positives to show in the left panel.
        fp_seed: Tie-breaking seed for the top-FP selection (rebuild-stable).
        cell_size: Inches per column in the reference matrix (controls figure width).
        ui_scale: Global font/size scale.
        page_aspect: Figure width/height ratio; default suits a full landscape A4 page.
        lang: Band label language: ``en`` (preprint default) or ``fr`` (report annex).
        suptitle: Figure-level super-title text.
        macros_prefix: Prefix for the LaTeX macro names (``ExpOneMatrix`` ->
            ``\\ExpOneMatrixPlants`` etc.).  Use distinct prefixes per arm/experiment
            to prevent duplicate-command collisions when multiple files are ``\\input``
            together.
    """
    if not data.cells:
        log.warning("render_recognition_matrix: no cells — skipping output")
        return
    ...
    fig.suptitle(suptitle, fontsize=20 * ui_scale, y=1.02)
    ...
    if output_macros is not None:
        ...
        output_macros.write_text(
            f"% Auto-generated by aedist.plot_exp1_matrix — do not edit.\n"
            f"\\newcommand{{\\{macros_prefix}Plants}}{{{n_plants}}}\n"
            f"\\newcommand{{\\{macros_prefix}Runs}}{{{n_runs}}}\n"
            f"\\newcommand{{\\{macros_prefix}FPs}}{{{n_fps}}}\n",
            encoding="utf-8",
        )
