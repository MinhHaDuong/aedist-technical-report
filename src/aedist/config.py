...
# Source-concordance table (ticket 0486) — generated by
# tabulate_source_concordance.py (reference vs GEM + Wikipedia, by status).
# Single source of truth; do not hardcode the filename outside this module.
SOURCE_CONCORDANCE_CSV = (
    _REPO_ROOT / "data" / "reference" / "tab_source_concordance.csv"
)

# Wikipedia recall bar (ticket 0494) — generated by
# tabulate_wikipedia_recall_bar.py (per-raw-status seeded-ceiling coverage).
# Single source of truth; do not hardcode the filename outside this module.
WIKIPEDIA_RECALL_BAR_CSV = (
    _REPO_ROOT / "data" / "reference" / "tab_wikipedia_recall_bar.csv"
)

# Long-tail layer-membership table (ticket 0514) — generated by
# plot_longtail_recognition.py: one row per reference plant with per-layer
# documentation flags (Gold, GEM, Wikipedia, OSM). The long-tail recognition
# figure reads THIS CSV; caption counts are re-derived from it
# (derive-prose-from-artifacts rule). Single source of truth.
LONGTAIL_LAYERS_CSV = (
    _REPO_ROOT / "data" / "reference" / "tab_longtail_layers.csv"
)

# OpenStreetMap power=plant extract for Vietnam (Overpass API fetch) —
# columns: name, source (fuel), capacity string ("660 MW"), lat, lon.
# Recognition layer for the long-tail figure (ticket 0537): a presence check
# for physically existing infrastructure only. Directory name contains a space.
OSM_VN_POWER_PLANTS_CSV = (
    _REPO_ROOT / "data" / "reference" / "OSM fetch" / "vn_power_plants.csv"
)
