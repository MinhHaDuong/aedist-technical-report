...
Reads per-run metrics from rag condition and vote results from
rag_consistency, groups by model, computes medians at render time,
...
def generate_per_run_table(results: list[dict]) -> str:
    ...
    lines = [
        ...
        "\\caption{Per-run breakdown: rag, 5 models $\\times$ 3 runs"
...
def assemble_from_measurements(metrics: list[dict]) -> list[dict]:
    ...
    - rag/deepseek-v3.2-run1  (per-run)
    - rag_consistency/deepseek-v3.2-consolidated  (majority vote)
    - rag_consistency/deepseek-v3.2-union  (union vote)
    """
    ...
    for m in metrics:
        label = m.get("label", "")
        if label.startswith("rag_consistency/"):
