...
def render_tex(cells: dict, f1_eff: dict, cost_eff: dict) -> str:
    """A 2x2 LaTeX table: F1 (top) and cost ratio (bottom) per cell, with
    marginal factor effects and a coverage/consistency note."""

    def eff_line(label: str, fe: dict, ce: dict, f1_unit: str, cost_unit: str) -> str:
        f1s = f"{fe['effect']:+.3f}{f1_unit} ({fe['k_positive']}/{fe['n']})"
        cs = f"{ce['effect']:.2f}{cost_unit} ({ce['k_positive']}/{ce['n']})"
        return f"{label} & {f1s} & {cs} \\\\"
    ...
    lines = [
        ...
        " & Without docs & With docs \\\\",
        "\\midrule",
        f"Single query & {_fmt_f1(cells['naive'])} & {_fmt_f1(cells['arm3'])} \\\\",
        f"             & {_fmt_cost(cells['naive'])} & {_fmt_cost(cells['arm3'])} \\\\",
        f"Multi-turn   & {_fmt_f1(cells['optimised'])} & {_fmt_f1(cells['arm4'])} \\\\",
        ...
        eff_line("Documents", f1_eff["docs"], cost_eff["docs"], "", "$\\times$"),
        eff_line("Query mode (multi$-$single)", f1_eff["mode"], cost_eff["mode"], "", "$\\times$"),
        eff_line("Interaction", f1_eff["interaction"], cost_eff["interaction"], "", "$\\times$"),
        "\\bottomrule",
        "\\end{tabular}",
    ]
    return "\n".join(lines) + "\n"
...
def main(argv: list[str] | None = None) -> None:
    ...
    args.output_tex.write_text(render_tex(cells, f1_eff, cost_eff), encoding="utf-8")
