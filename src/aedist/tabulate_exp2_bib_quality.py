...
def summarize_bib_quality(rows: list[dict]) -> list[dict]:
    ...
    # Sort by arm order (naive first) then agent order
    arm_order = {"naive": 0, "optimised": 1}
    summaries.sort(
        key=lambda r: (
            arm_order.get(r["arm"], 2),
...
def generate_latex(summaries: list[dict]) -> str:
    """Render summary rows as a LaTeX longtable."""
    lines = [
    ...
    for row in summaries:
        arm_display = row["arm"].capitalize()
        if row["arm"] == prev_arm:
            arm_display = ""
        else:
        ...
        lines.append(
            f"{arm_display} & {agent_display}{valid_note} & "
            f"{row['n_rows']} & "
