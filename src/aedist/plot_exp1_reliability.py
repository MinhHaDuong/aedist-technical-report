...
def write_annex_table(cells: list[dict[str, str]], output: Path) -> None:
    ...
    # Ticket 0598: the Spearman ρ column is dropped — the regression is no
    # longer presented anywhere in the manuscript; the sweep reports only the
    # disqualified-count stability that the annex prose discusses. The CSV
    # artifact still carries the `spearman` field for the record.
    lines = [
        "% Generated by aedist.plot_exp1_reliability — do not edit.",
        r"\begin{tabular}{llrrc}",
        r"\toprule",
        r"Indicator set & $n$ dims & $\tau$ & \#disqualified & = floor \\",
        r"\midrule",
    ]
    prev_set = None
    for cell in cells:
