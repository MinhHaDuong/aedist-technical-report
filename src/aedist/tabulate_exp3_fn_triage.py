...
def generate_summary(
    ...
    if total == 0:
        raise ValueError(
            "No classified rows found in the worksheet. "
            "Fill in the 'bucket' column before running --mode summary."
        )
...
def _generate_latex(summary_rows: list[dict], total: int) -> str:
    ...
    lines = [
        ...
        "\\begin{table}[htbp]",
        "\\centering",
        "\\caption{Triage des faux n\\'{e}gatifs (arm3/GPT-5.5/run4,"
        " $n=" + str(total) + "$~FN; \\S{}TODO: update experiment label)}",
