...
(7~modules, 2~bras) et le \textbf{régime d'information} (4~modes d'accès
...
\item[Base] Le prompt de base DSPy (section~\ref{sec:prompt-base},
  optimisé par MIPROv2, voir Phase~0), point de référence intermédiaire.
...
Quatre régimes d'accès aux connaissances sont testés en croisant les 18~prompts~:
...
\item[Recherche web] Le modèle dispose d'un outil de recherche Internet
  (Tavily ou plugin natif).
\item[Incrémental] Le modèle part de sa sortie paramétrique (table initiale),
  puis reçoit les 18~documents un par un~: chaque tour fournit un document
  et demande de mettre à jour la table. Le régime mesure la valeur marginale
  de chaque document ajouté et la capacité du modèle à réviser sans régresser.
  Coût~: $18\times$ plus de tours que les autres régimes~; exécuté sur un
  sous-panel de modèles (Phase~3).
\end{description}

\subsection{Protocole}\label{sec:protocole-ablation}

\paragraph{Phase~0 — Optimisation DSPy du prompt base}

Le prompt base manuel (section~\ref{sec:prompt-base}) est optimisé par
MIPROv2 sur un ensemble d'entraînement disjoint de l'ensemble d'évaluation
(répartition 60/40 de l'inventaire de référence). Le prompt résultant~---
\emph{prompt base DSPy}~--- remplace le prompt base manuel pour toutes les
phases ultérieures. Les résultats H$_1$ (section suivante) restent valides
comme référence sur le prompt base manuel~; ils mesurent le gain dû à
l'ingénierie de prompt humaine, indépendamment de l'optimisation automatique.

\paragraph{Contrôle de la variance}

Tous les appels sont effectués avec $t{=}0{,}0$, graine fixée (\texttt{seed=42})
et fournisseur épinglé via OpenRouter (\texttt{provider\_order}). Ces trois
verrous éliminent la variance stochastique~: deux runs identiques devraient
produire des sorties bit-à-bit identiques. \textbf{Vérification préalable
obligatoire~:} avant chaque phase, deux répétitions sont lancées sur un modèle
pilote~; si les sorties sont identiques, une seule répétition suffit pour le
reste de la campagne. Si le fournisseur n'honore pas le seed (divergence
observée), on revient à 3~répétitions pour ce modèle ou on l'exclut du panel.
Cette preuve de déterminisme est reportée en annexe et constitue une garantie
de reproductibilité pour les relecteurs.

\paragraph{Phase~1 — Sélection des modèles}

Les prompts~1 (base DSPy) et~9 (complet) sont exécutés sur l'ensemble des
modèles du census (25+~modèles cloud), \textbf{2~répétitions pilotes} à
$t{=}0{,}0$ sur les régimes paramétrique et RAG. Si le déterminisme est
confirmé, une seule répétition est retenue par condition. On retient les
$k$~modèles dont le $|\Delta\text{F1}|$ moyen dépasse 0,05 (5~points) sur
au moins un régime.

\paragraph{Phase~2 — Mesure complète (régimes paramétrique, RAG, livesearch)}

Les 18~prompts sont exécutés sur les $k$~modèles retenus à $t{=}0{,}0$,
\textbf{1~répétition} si le déterminisme est confirmé en Phase~1, sinon~3
($18 \times k \times 1$ à $18 \times k \times 3$~runs par régime). Chaque
run produit un CSV évalué contre l'inventaire de référence (163~centrales,
matching par nom exact et approximatif). Le nombre total de tokens générés
est mesuré par condition pour contrôler l'effet de budget de sortie.

\paragraph{Phase~3 — Régime incrémental}

Sur un sous-panel de $k'{\leq}k$~modèles~: les 4~prompts d'ancrage (Census,
Base DSPy, Complet, Frontier) sont exécutés en régime incrémental
(18~tours par run, \textbf{1~répétition} si déterminisme confirmé). Les
14~variantes de composition/ablation ne sont pas répétées en régime
incrémental sauf si Phase~2 révèle des effets d'interaction forts.
...
Phase~0 (DSPy)~: inclus dans le budget MIPROv2 (ticket~0075).
Phase~1 (sélection)~: $2 \times 25 \times 2 \times 2 = 200$~runs pilotes,
${\approx}\,3$~\$.
Phase~2 (mesure, $k{=}5$, déterminisme confirmé)~: $18 \times 5 \times 1 \times 3 = 270$~runs,
${\approx}\,5$--15~\$.
Phase~3 (incrémental, $k'{=}3$, déterminisme confirmé)~: $4 \times 3 \times 1 \times 18 = 216$~tours,
${\approx}\,2$--5~\$.
Total (cas favorable)~: ${\approx}\,10$--23~\$~; cas défavorable (déterminisme non confirmé)~:
${\approx}\,25$--70~\$. Les modèles open-weight réduisent ce budget d'un facteur~5--10.
...
\item \textbf{Plan fractionnaire (résolution~III).} Le design teste les
  ...
  spécifique.
\item \textbf{Régime incrémental limité.} Pour des raisons de coût, le régime
  incrémental n'est appliqué qu'aux prompts d'ancrage en Phase~3. Les effets
  d'interaction module~$\times$~incrémental ne sont pas mesurés sauf si
  Phase~2 les suggère.
\item \textbf{Dépendance à DSPy.} Si MIPROv2 absorbe l'effet d'un module en
  l'intégrant au prompt base optimisé, son $\Delta$F1 en ablation sera proche
  de zéro. Cet effet est attendu pour les modules à fort signal de rappel
  (cadrage, sourçage-ground).
\end{itemize}
...
point a posteriori)~; (b)~le design a été révisé (7~modules, prompt base DSPy,
4~régimes d'information). Les figures seront régénérées après la Phase~2
révisée. Les résultats pilotes sont conservés ici à titre d'orientation
...
à confirmer en Phase~2 révisée.
