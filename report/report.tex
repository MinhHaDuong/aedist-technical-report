...
\begin{figure}[h]
\centering
\includegraphics[width=\linewidth,keepaspectratio]{inputs/generated/fig_exp2_arms_comparison.pdf}
\caption{Expérience~2 (frontière SOTA)~: couverture (nombre d'actifs identifiés, en rouge les faux positifs) et coût API par exécution, regroupés par agent. Les barres distinguent les deux bras enregistrés (sans documents) des deux conditions exploratoires (avec documents). Quatre agents, 5~répétitions par cellule.}
...
La figure~\ref{fig:recognition-matrix} aligne chaque centrale de la liste de
référence sur une colonne fixe, pour les \ExpOneMatrixRuns{}~runs de
l'Expérience~1 (les 14~modèles à raison de cinq répétitions chacun, dans
l'ordre de la figure~du corps de texte~: par famille architecturale puis par
nombre effectif de paramètres décroissant). Une case bleue indique que le run a
reconnu la centrale~(vrai positif)~; une case vide marque un oubli. Les
\ExpOneMatrixPlants{}~colonnes de référence sont ordonnées par groupe de statut
puis par capacité décroissante, de sorte que les bandes de statut s'alignent sur
le tableau de difficulté par statut~(table~\ref{tab:status-difficulty}). Le
panneau de gauche
montre les \ExpOneMatrixFPs{}~faux positifs les plus fréquents sur l'ensemble des
runs, triés par occurrence décroissante~; le rouge suit la convention faux
positif. Contrairement à la figure du corps de texte qui tasse les centrales
reconnues vers la gauche, l'alignement vertical révèle quelles centrales chaque
modèle manque~: les centrales célèbres forment des colonnes denses, les
obscures des colonnes clairsemées.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{inputs/generated/fig_exp1_recognition_matrix.pdf}
\caption{Matrice de reconnaissance de l'Expérience~1~: \ExpOneMatrixPlants{}~centrales
de référence (colonnes, par statut puis capacité décroissante) contre
\ExpOneMatrixRuns{}~runs (lignes). Bleu~= centrale reconnue~; panneau de gauche~:
les \ExpOneMatrixFPs{}~faux positifs les plus fréquents (rouge).}
\label{fig:recognition-matrix}
\end{figure}

\input{inputs/generated/tab_status_difficulty}
