...
\small
\textbf{Synchronisation en cours.} L'introduction de ce chapitre décrit le RAG comme point d'arrivée de l'extraction. Le cadrage v0 courant le recadre comme \emph{composant de récupération} à l'intérieur d'un pipeline de fusion incrémentale (voir \texttt{MASTERPLAN.md} §``Data model (v0 table fusion)''). Les résultats expérimentaux de ce chapitre (sections RAG wholesale, décomposition, extraction sourcée, cohérence) restent valides et inchangés. Réécriture de l'introduction suivie dans le ticket~0098. (2026-04-17)
\end{minipage}}
\end{center}
...
\fbox{\begin{minipage}{0.92\textwidth}
\small
\textbf{Synchronisation en cours.} Ce chapitre décrit une architecture à quatre couches et quatre agents au niveau vision. Le modèle de données v0 courant (master.csv + master\_provenance.csv en lockstep, cinq invariants validés, registre des sources alimenté au triage HITL, journal de fusion, vérification à trois niveaux, mémoire ratifiée avec piste d'audit) est spécifié dans \texttt{MASTERPLAN.md} §``Data model (v0 table fusion)''. Les slides Econom'IA~2026 présentent le cadrage courant. Réécriture complète du chapitre suivie dans le ticket~0098 (bloqué par ticket~0097 pour les résultats de grounding). (2026-04-17)
\end{minipage}}
\end{center}
