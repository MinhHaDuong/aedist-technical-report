...
\section{Expérience 3~: RAG wholesale à l'échelle (sweep~2)}\label{sec:sweep2-rag}
...
Les résultats de ce rapport montrent qu'un pipeline RAG avec décomposition des requêtes récupère déjà 99\,\% d'un inventaire expert, pour un coût marginal. Le 1\,\% restant -- filtrage des hallucinations, résolution d'entités, traçabilité des sources -- relève de l'ingénierie de la connaissance plutôt que d'un verrou scientifique. Le cas du Vietnam constitue un premier benchmark~; les méthodes proposées sont transposables à d'autres pays et à d'autres secteurs nécessitant un suivi rigoureux des transformations socio-techniques.

Parmi les axes non explorés ici, le réglage de l'effort de raisonnement (reasoning effort) des modèles capables de « réfléchir plus longtemps » (o3, DeepSeek R1, QwQ) pourrait améliorer l'extraction sur les cas difficiles à faible coût additionnel.

Le développement rapide des modèles de langage et des frameworks agentiques (de nouveaux outils apparaissent chaque mois) laisse prévoir que les lacunes identifiées -- en particulier l'intégration RAG-graphe et la construction automatique d'ontologies -- seront progressivement comblées. L'enjeu est de constituer dès maintenant les bases documentaires et les référentiels de qualité qui permettront d'exploiter pleinement ces avancées.
