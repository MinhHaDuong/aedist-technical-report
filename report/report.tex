...
Les résultats de cette étude pilote montrent qu'un pipeline RAG avec décomposition des requêtes récupère déjà 98,8\,\% d'un inventaire expert, pour un coût marginal de 0,06\,\$. Le design de la méthode -- régime documentaire, décomposition, agrégation -- explique davantage de variance que le choix du modèle~: un modèle open-weight à 0,26\,\$/Mtok surpasse des modèles propriétaires dix fois plus coûteux. La précision atteinte sur le cas pilote vietnamien est encourageante~; la barrière du coût est levée.
