...
Une expérience de vérification automatisée quantifie ce compromis~: sur la meilleure extraction (DeepSeek~V3.2, décomposition, F1\,=\,98,8\,\%, 167~lignes), la confrontation de chaque ligne avec la base de référence (vérification outil) identifie 102~centrales confirmées et 65~non trouvées. Au seuil d'évidence~$\geq 3$ (au moins une source primaire), la précision atteint 100\,\% — zéro fabrication — mais la couverture chute à 62,6\,\% (F1\,=\,77,0\,\%). Les 65~centrales écartées ne sont pas des erreurs d'extraction~: la base de référence est incomplète. Ce résultat illustre le coût de la confiance~: exiger la traçabilité des sources élimine les hallucinations mais au prix d'une perte de couverture qui reflète les lacunes du corpus de validation, non celles du pipeline. Le tableau~\ref{tab:verification} détaille ce compromis par seuil d'évidence.

\begin{table}[htbp]
\centering
\caption{Compromis précision-couverture par seuil d'évidence. Vérification outil (confrontation à la base de référence) sur l'extraction DeepSeek~V3.2 décomposée (167~lignes).}
