...
\textbf{Abstract.} The binding constraint on producing trustworthy
statistical registers with AI is the quality of the source documents,
not the capability of the model. Benchmarking the task on Vietnam's 177
thermal power plants --- the asset-level data that open energy-system
models need but that arrives late, incomplete, or under restrictive
licences across much of the developing world --- fourteen language
models drawing solely on memorised knowledge (70 runs) achieve row-level
F1 from 0.00 to 0.68 (mean 0.38), well below an available ceiling: the
reference content sat in the training corpus for years, and the models
still fail to retrieve it. Attribute classification is uneven: province
is recovered well, fuel and status remain mediocre. Four frontier agents
with web access and extended reasoning improve coverage but still fall
short of the four-dimensional quality bar --- accuracy, coherence,
provenance, temporality --- consistently and reproducibly. However,
providing agents with curated documents narrows the performance spread
across models (max−min F1 from 0.26 to 0.18), equalising weak and
strong: which documents are supplied matters more than which model runs
the query. Within-run capacity variability predicts reference-based F1
at Spearman ρ = 0.92 pooled (within-model signal positive but modest,
\ref{sec:annex-screen}), offering a reference-free screen that rejects
the weakest outputs without ground truth. Pooling the 14-model outputs
already recovers plants no single model reliably finds. The results
point toward a sourced, auditable knowledge base from which statistical
tables derive as snapshot projections --- investing in the corpus and
the sourcing process, not in a stronger model. To our knowledge, no
prior benchmark targets open-world enumeration of a national asset class
with per-cell provenance at this granularity.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{Introduction}\label{sec:intro}

Open energy-system models such as PyPSA-ASEAN need power-plant
...
analysis in countries where the energy landscape changes faster than
...
whether AI systems can yet produce research-grade statistical data. We
situate the work against the empirical landscape of open power-plant
databases (§\ref{sec:related-empirical}), define a four-dimensional
quality bar (§\ref{sec:quality}), survey the AI capability landscape
(§\ref{sec:capability}), measure what model memory alone delivers across
14 language models (§\ref{sec:exp1}), test the commercial frontier of
mid-2026 (§\ref{sec:exp2}), and show that targeted pipeline design is
needed to bridge the gap (§\ref{sec:fusion}). The Discussion
(§\ref{sec:discussion}) examines the limits of F1 as an aggregate and
prefigures a fusion-based research programme.
...
\item
  ...
  of AI agents on national power-plant inventory construction, with
  per-cell provenance against a held-out reference.
\item
...
\section{Related Work --- Empirical
landscape}\label{sec:related-empirical}
...
reference is built afresh rather than adopted from GEM. Four reasons.
\emph{Per-cell provenance}: the research-grade bar (§\ref{sec:quality})
demands that each value (capacity, status, commissioning date) trace to
a specific primary source, whereas aggregate trackers carry source
attribution at the record level at best. \emph{Under-documented sites}:
GEM and WRI cover documented, large-capacity units well; the
forward-looking PDP8 pipeline of proposed and planned LNG and gas
projects, much of it in Vietnamese-language master-plan annexes, is
exactly the under-documented tail they thin out. \emph{Reproducible
compilation}: a single-author manual compilation from primary sources
(PDP7/7A/8 annexes, EVN reports, MOIT decisions), version-locked and
accompanied by per-row source priority, is auditable end-to-end in a way
a redistributed aggregate is not. \emph{Licence}: the reference and its
provenance ship under CC BY 4.0, free of the redistribution constraints
that encumber several upstream sources. The long-tail figure makes the
documented-head / under-documented-tail split concrete
(Figure~\ref{fig:longtail}): the 177 reference plants, sorted by
visibility, are checked against four recognition layers, and the
speculative tail thins out from layer to layer.
...
source holds. The 177 plants, sorted by descending visibility, are
checked against four recognition layers: the hand-compiled reference
itself (all 177), the Global Energy Monitor tracker (157), Wikipedia
coverage (130), and the language-model census from §\ref{sec:exp1} (144
plants recognised by at least one of the 70 runs). Well-documented
operating plants appear in every layer; the under-documented tail ---
proposed and planned projects from the PDP8 pipeline --- thins out layer
by layer. This tail is the reference's unique contribution, and the
segment where investing in a sourcing pipeline adds the most value.
Counts re-derived from
...
audience-free definition; the reference is built as the superset so that
each downstream use can take the perimeter it needs, which is why the
speculative tail is kept rather than pruned.

The paper takes a problem the three aggregators do not: extracting a
complete, sourced inventory for a country where the facts are dispersed
across heterogeneous documents, partly in a non-Latin script, and where
no authoritative single-source aggregate exists. That problem ---
open-world enumeration of a national asset class with per-cell
provenance, against a held-out reference --- motivates the benchmark and
the research programme that follow.

\section{Quality dimensions for research-grade statistical
datasets}\label{sec:quality}
...
\item
  ...
  the right attributes. At the row level, this means recall and
  precision: does the system find all relevant assets, and does it
  exclude non-assets or duplicates? For a simple inventory table, this
...
\section{AI capability landscape: from chatbots to agentic
systems}\label{sec:capability}
...
delegated sub-tasks is what lets it leverage each surface when the
...
inventories.} Across 70 runs (14 models × 5 repeats), all produced
usable tables, but row-level F1 ranges from 0.00 to 0.68, with a mean of
0.38. Asking the same model the same question does not return the same
...
province accuracy is high (mean 0.89) --- unsurprising, since Vietnamese
plants are typically named after the places that host them, so the name
itself usually gives away the province --- but fuel (0.54) and status
(0.57) remain mediocre, reflecting the finer coal-versus-gas and
...
status accuracy. Paying more does not reliably buy a better inventory:
...
performance across 14 models and 70 runs on the 177-plant Vietnam
...
green line marks the 177-plant reference; the light-grey dotted line
marks Wikipedia's light-reviewed coverage of the reference (73\%) ---
...
dashed line at 177 marks the full Vietnam thermal inventory. Marker
...
177-plant reference over the good runs; for the three models with zero
...
\section{Experiment 2: the commercially available frontier still falls
short}\label{sec:exp2}
...
Monitor, covers only 89\% of the reference after light review
...
(N=5 each). \textbf{Arm 1 (naive)} --- a single-shot prompt (Doc-07)
with no scaffolding, web on; the null comparator. \textbf{Arm 2
...
raw model capability. Row-level F1 against the 177-plant reference is
...
Values re-derived from
\texttt{experiments/derived/tab\_exp2\_2x2.csv}.}\tabularnewline
\toprule\noalign{}
...
bars (blue, upward, matched against the 177-plant reference) and FP bars
...
points. Dashed green line marks the 177-plant reference count; the
light-grey dotted line marks GEM's light-reviewed coverage (89\%), the
best independent external database (\ref{sec:annex-exp1}). Row-level F1
against the 177-plant reference is now scored for all 80 runs; the 2×2
...
against the 177-plant reference. The optimised arm (arm 2) does not
...
capability to document quality. The leverage is therefore in the corpus
...
granularity. To our knowledge, the conjunction of per-cell provenance
with per-cell temporal validity in LLM-augmented inventories has not
been published; demonstrating it is part of the contribution we aim for
in the follow-on paper.

\textbf{Pooling runs by simple union more than doubles recall, at no new
collection cost.} The 14-model exp1\_batch2 cohort (a subset of the
...
\section*{Related Work --- Methods}\label{related-work-methods}
...
produce research-grade statistical inventories reliably --- and the
binding constraint is the data, not the model. A baseline across 14
models answering from memory finds row-level F1 from 0.00 to 0.68 with
high within-model variance --- below an available ceiling, since the
group-seeded Wikipedia lists alone cover 92\% of the built fleet
(§\ref{sec:exp1}): the knowledge was demonstrably in the training
corpus, and the models failed to retrieve it. Frontier agents with web
access and extended reasoning improve coverage but fail to clear the
four-dimensional quality bar consistently; handing the same agents a
curated document set narrows the gap between weak and strong models more
than any model choice does (§\ref{sec:exp2}). Two levers cost nothing in
new data collection: an internal coherence screen --- within-run
capacity variability, ρ = 0.92 pooled with reference-based F1
(within-model signal positive but modest, \ref{sec:annex-screen}) ---
rejects the weakest outputs without ground truth, and pooling runs
across models recovers plants that no single model reliably finds
(§\ref{sec:fusion}). The path to research-grade quality therefore runs
through the sourcing pipeline --- assembling, curating, and resolving
documents in an auditable knowledge base from which statistical tables
derive as dated snapshots --- with human-in-the-loop resolution and
multi-run fusion, not through a stronger model or a single heroic
prompt. For the many countries whose statistical systems cannot yet
supply machine-readable asset inventories, that is where the investment
should go.
...
\section{Temporality: state representations vs.~event
representations}\label{sec:annex-temporality}
...
177 plant-level records covering coal (85) and gas/gas-oil (92), across
...
provenance}\label{reference-dataset-provenance}
...
external check is Global Energy Monitor (GEM, 119 distinct plants;
snapshot exported no later than 2026-04-02, the date it entered version
control), against which we report a light-reviewed concordance below.

\textbf{Source concordance (reference vs GEM and Wikipedia).} GEM covers
157 of the 177 reference plants (89\%) after light review --- more
matches than GEM's 119 distinct plants because matching operates at the
reference's phase grain: one GEM plant row can match several reference
phase rows (e.g.~successive phases of the same site ---
\texttt{gem\_thermal.csv} has 153 rows under 119 distinct plant names).
Wikipedia (coal list + general-page gas section) covers 130 (73\%).
...
\textbf{All} & \textbf{177} & \textbf{157} & \textbf{12} &
\textbf{130} \\
\end{longtable}

\emph{Table: source concordance by status, generated from
\texttt{data/reference/tab\_source\_concordance.csv}. Coverage is
\textbf{light-reviewed} --- an ASCII-fold pass recovers
...
pinning, characterised in earlier in-project work; the 5-repeat budget
...
\subsection*{Post-fix top-up cohort and interday
variability}\label{post-fix-top-up-cohort-and-interday-variability}
...
\subsection*{Sweep configuration}\label{sweep-configuration}
...
\subsection*{What this experiment does and does not
test}\label{what-this-experiment-does-and-does-not-test}
...
population as Experiment 1: the 177-plant version-locked reference
inventory at
\texttt{data/reference/vietnam\_thermal\_plants\_v2\_classified.csv}.
...
\subsection*{Baseline reference for
cross-comparison}\label{baseline-reference-for-cross-comparison}
...
\subsection*{Phase structure}\label{phase-structure}
...
\subsubsection*{Phase B dialogue policy (fixed experimental
condition)}\label{phase-b-dialogue-policy-fixed-experimental-condition}
...
\subsection*{Arms and budget}\label{arms-and-budget}
...
\subsection*{As-run deviations from the
registration}\label{as-run-deviations-from-the-registration}
...
Phase B outputs are evaluated against the 177-plant reference using the
...
\subsection*{What this experiment does and does not
test}\label{what-this-experiment-does-and-does-not-test-1}
\addcontentsline{toc}{subsection}{What this experiment does and does not
test}
...
\section{Experiment 1: per-plant recognition matrix and the status
composition of task difficulty}\label{sec:annex-recognition}

Figure~\ref{fig:recognition-matrix} places each of the 177 reference
plants on its own row (names read left-to-right, no rotation) against
the 70 Experiment 1 runs (14 models × 5 repetitions) as columns. A blue
...
stage. A final page lists the 40 most frequent false positives across
...
\emph{Table: composition of the reference list by status, and mean
recognition rate (Experiment 1, direct method: 14 models × 5
repetitions). The rate is the share of run × plant cells recognised
among plants of that status.}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{Run-grain screen validation: within-model accuracy
gap}\label{sec:annex-screen}
...
surviving runs): mean of per-model gaps = \textbf{+0.162} (deepseek:
+0.384, gpt-oss-120b: +0.078, qwen3.6-flash: +0.022). The reduction from
the pooled gap (+0.335) to the within-model gap (+0.162) shows that
roughly half the pooled signal was attributable to the across-model
confound; the residual within-model effect (+0.162) is the honest lower
bound.
...
(10/14), supporting the claim that the screen earns its keep at the run
grain. However, the statistic is modest, and only 3 of the 14 models
...
One false-veto case exists: qwen3.6-flash run 1 (F1 = 0.212, the model's
best run) is vetoed by the cap\_distinct threshold while two
lower-scoring runs survive. These limitations are acknowledged; the
...
\section{Capability rollout: per-lab shipping
order}\label{sec:annex-rollout}
