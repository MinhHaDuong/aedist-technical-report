...
*Figure 4. Quality-floor heatmap for Experiment 1 (parametric arm, 12 models × 5 runs). Rows are models in four architectural families (Claude, GPT, Mistral, Qwen); DeepSeek is excluded (no panel in the quality taxonomy). Columns are the sixteen sub-score criteria across five dimensions (Accuracy, Coherence, Field completeness, Provenance, Temporality), derived programmatically from the scoring CSV, plus a single coherence-veto column. A sub-score cell is **red** iff a majority (≥ 3 of 5) of that model's runs scored zero on that criterion — indicating a systematic failure. The **coherence-veto** cell is red iff a majority of the model's runs are vetoed by the §4 internal-coherence screen (`cap_distinct ≤ 4 OR status_distinct ≤ 1`, the reference-free ρ=0.92 screen): the flag is inverted (a vetoed run is the bad case), so it is colored on a majority-vetoed rule rather than on zeros. Any red cell in a row disqualifies the model: it cannot clear the §2 quality bar, which is a conjunction — a zero on any criterion, or a majority coherence veto, means the bar is not cleared. Model labels and family-separation lines help the reader isolate which families clear all criteria and which do not.*
