...
![Figure 3](inputs/generated/fig_exp2_arms_comparison.pdf)
...
![Figure 4](inputs/generated/fig_exp2_turn_trajectory.pdf)

*Figure 4. Experiment 2 Arm 2 (optimised) — inventory rows per turn, N=5 runs per agent. Filled circles denote turns classified as report; open circles denote no\_report. Y-axis is shared across all four panels.*

**Observed results (operational metrics).** All twenty optimised-arm runs produced a report; the naive arm had four no\_report failures (Anthropic runs 3 and 5, Mistral runs 2 and 3), confirming that the multi-turn protocol improves output reliability. Coverage diverges sharply by agent. Anthropic gains the most: the optimised arm enumerates a median of 135 plants per run versus a median of 18 in the naive arm, a five-fold increase. OpenAI is already strong in the naive arm (median 126 rows) and improves modestly under the optimised protocol (median 159). Mistral is unreliable naive (3/5 reports, median 52 among those) and becomes fully reliable optimised, though with a lower median row count (22), suggesting the multi-turn state machine trades breadth for completeness. Qwen is the outlier: the naive arm produces a consistent 69–79 rows across all five runs, while the optimised arm drops to 13–18 rows — the self-designed multi-turn prompt hurt coverage for this agent. Figure 4 traces this regression to its origin: Qwen's Phase A self-designed protocol imposed a strict dual-source admissibility filter (PDP8 → EVN → ERAV hierarchy) that already constrains Turn 1 to 14–19 rows; the verify turn compresses prose but does not change row counts. For Anthropic and OpenAI, by contrast, Turn 1 classifies as no\_report (exploration) and the table grows through successive turns, reaching 128–215 rows by the final turn. Output length (panel B) follows a similar pattern for Qwen (6–8 k chars optimised vs 19–23 k naive) and confirms the regression is not a classification artefact. The optimised protocol costs 2–4× more per run than naive across all agents; OpenAI's ratio is most extreme ($0.95 median optimised vs $0.27 naive). We did not find published comparisons of frontier deep-research agents on structured-output coverage at this granularity; the agent-by-agent divergence — protocol gain for Anthropic and OpenAI, regression for Qwen — is the primary empirical finding of this experiment.

**Observed results (provenance quality).** Beyond row counts, every Source 1, Source 2, and Notes cell across all 40 runs was parsed to measure citation coverage and source quality (full methodology and table in the technical analysis report, `docs/exp2-analysis-v0.md` §3.5). OpenAI leads on source compliance in the optimised arm: 88% of Source 1 citations trace to primary-tier documents (government filings, operator annual reports, PDP-series) and the agent produces a mean 39 bibliography entries per run. The naive arm demonstrates the scaffold's contribution: without the optimised protocol's explicit source-quality taxonomy, Mistral cites a source in only 4% of rows and Claude's citations are predominantly secondary or tertiary (19% primary rate). Qwen's coverage regression has a direct provenance explanation — its self-designed Phase A protocol imposed a strict PDP8→EVN→ERAV admissibility filter, trading breadth for citation discipline: 15 rows at 55% primary vs. 71 rows at 73% primary naive. Across all agents and arms, a coverage–certainty tradeoff is visible: agents that enumerate more plants tend to cite lower-quality sources (Figure 6). To our knowledge, per-cell provenance analysis of this kind has not been published for frontier deep-research agents.

![Figure 6](inputs/generated/fig_exp2_coverage_certainty.pdf)

*Figure 6. Coverage–certainty tradeoff across 33 Exp2 runs (7 excluded: 5 with zero parsed inventory rows, 2 with no Source 1 citations). X-axis: inventory rows enumerated (coverage proxy). Y-axis: fraction of Source 1 citations traced to primary-tier documents (government filings, operator reports, PDP-series). Filled circles: optimised (multi-turn) arm; open circles: naive (single-shot) arm. Colours encode agents. OpenAI occupies the upper-right quadrant (high coverage, high certainty); Qwen's optimised arm has the lowest coverage (10–19 rows) with primary rates spanning 0–100%.*
