...
**Results.** Across 70 runs (14 models × 5 repeats), all produced usable tables. Row-level F1 ranges from 0.00 to 0.68, with a mean of 0.38. Within-model F1 variance is large: DeepSeek V4-Flash ranges from 0.07 to 0.52 across 5 identical runs — a spread wider than the gap between many adjacent model means. Even for correctly matched plants, attribute classification is uneven: province accuracy is high (mean 0.89) — a model that recalls a plant usually places it in the right province — but fuel (0.54) and status (0.57) remain mediocre, reflecting the finer coal-versus-gas and lifecycle distinctions the models conflate. No monotonic relationship between API cost and F1 is observed: Claude Opus 4.6 at \$1.23 total achieves mean F1 = 0.66, while GPT-OSS-20B at \$0.01 reaches mean F1 = 0.02. To our knowledge, this non-monotone cost-versus-F1 relationship for structured factual extraction has not been documented in prior literature. The total experiment cost is \$2.85. As an external recall yardstick, the public Wikipedia coal-and-gas lists cover 73% of the reference after light review (Annex B) — a parametric ceiling against which the §4 results can be read.

![](../report/inputs/generated/fig_direct_p1_base.pdf)\

*Figure 2. Direct-query performance across 14 models and 70 runs on the 177-plant Vietnam thermal reference. Each bar is one run; family-coloured segments to the right are correctly identified plants (TP), red segments to the left are unrecognized plants (FP). The dashed green line marks the 177-plant reference; the light-grey dotted line marks Wikipedia's light-reviewed coverage of the reference (73%), an external recall ceiling (Annex B). Models are grouped on the vertical axis.*
