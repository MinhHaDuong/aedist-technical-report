...
Features 1 and 2 ship sequentially within each lab and within ~12 months across the industry. Features 3 and 4 emerge in parallel: within each lab the per-product order is not always 3 then 4 (Anthropic and Mistral both shipped code execution before or simultaneously with browsing), and the *cross-lab* shipping windows overlap substantially. Feature 4 (code execution) was the earliest tool-use surface shipped as a built-in consumer feature, predating the general MCP-like surface of feature 6 by ~18 months at OpenAI. Feature 7 (deep research) draws on both features 3 and 5 — at every lab where it shipped as a product, deep research arrived within 1–5 months of having both browsing and reasoning in place. DeepSeek is the negative case that confirms the framework-not-model framing: it ships features 3 and 5 (Internet Search in V2.5; R1 reasoning) and the model components that would enter a deep-research loop (V3.1's search-agent claims, V3.2's thinking-in-tools), but has not packaged the composer as a public product within the cutoff.
...
![*Figure 5. Empirical capability transition matrix across N=5 labs. Each cell shows the fraction of labs where feature i (row) shipped before feature j (column), conditional on both being present; N per cell. White = no lab made that transition; dark green = all labs did. Features 3 and 4 (browsing × code execution) show a three-way split — genuinely parallel. Feature 5 (reasoning) follows features 1–3 consistently. Feature 7 (deep research) draws on features 3 and 5. Descriptive historical record, N=5 labs — no statistical inference.*](inputs/generated/fig_capability_dag.pdf)
