...
Figure 7 places each of the 177 reference plants on its own row — names read left-to-right, no rotation — against the 70 Experiment 1 runs as columns (14 models × 5 repetitions). The columns carry a three-level header (architectural family, then model version, then repetition r1–r5), ordered as in Figure 2: by family, then by decreasing effective parameter count. A blue cell marks a recognized plant (true positive); an empty cell, a miss. Rows are grouped by status, then by decreasing capacity, with the status bands labelled in the right margin so they align with the difficulty table below. To keep plant names legible at print scale, the matrix is split across pages by lifecycle stage: terminal and active assets (operational, retired, cancelled) first, then pipeline assets (proposed, planned, under construction); a final page lists the 40 most frequent false positives across all runs, sorted by decreasing occurrence, in red (the paper's false-positive convention: unrecognized — possibly real, not necessarily fabricated). Unlike Figure 2, which packs recognized plants leftward, the fixed-row alignment reveals *which* plants each model misses: famous plants form dense rows, obscure ones sparse rows.

*Figure 7 (next three pages). Experiment 1 recognition matrix, transposed to portrait so plant names stay legible: the 177 reference plants are rows (grouped by status, then by decreasing capacity, with the status labelled in the right margin) and the 70 runs are columns under a three-level family / version / repetition header. Blue = plant recognized. Pages 1–2 split the plants by lifecycle stage (terminal and active assets, then pipeline assets); page 3 lists the 40 most frequent false positives (red).*
