...
![*Figure 4. Empirical capability rollout across the five labs in Experiment 1. Each row is a capability stage (1 chat LLM → 8 multi-agent recursion); each marker places when the named lab first shipped that stage in a consumer-facing product. Markers are coloured by architectural model family (claude / gpt / mistral / qwen / deepseek) from the project palette, and shaped by lab. The horizontal spread within each row is the cross-lab emergence window; vertical neighbours that overlap in time (notably stages 3 and 4) indicate parallel-not-sequential capability development. Source: `data/capability_timeline.csv`; per-lab primary announcements documented in `docs/capability-timeline.md`.*](inputs/generated/fig_capability_timeline.pdf)
...
Stages 1 and 2 ship sequentially within each lab and within ~12 months across the industry. Stages 3 and 4 emerge in parallel: within each lab the per-product order is not always 3 then 4 (Anthropic inverts; Mistral ships them within months), and the *cross-lab* shipping windows overlap. Stage 5 (deep research) is the empirical join of stages 3 and 4 — at every lab where the join was shipped as a product, deep-research followed within 2–7 months of having both prerequisites. Stage 6 (code execution) was the earliest tool-use surface shipped as a built-in consumer feature, predating the general MCP-like surface of stage 7 by ~18 months at OpenAI. DeepSeek is the negative case that confirms the framework-not-model framing: it ships stages 3 and 4 (Internet Search in V2.5; R1 reasoning) and the model components that would enter a deep-research loop (V3.1's search-agent claims, V3.2's thinking-in-tools), but has not packaged the composer as a public product within the cutoff.
