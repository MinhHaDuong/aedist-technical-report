...
Stages 1 and 2 ship sequentially within each lab and within ~12 months across the industry. Stages 3 and 4 emerge in parallel: within each lab the per-product order is not always 3 then 4 (Anthropic and Mistral both shipped code execution before or simultaneously with browsing), and the *cross-lab* shipping windows overlap substantially. Stage 4 (code execution) was the earliest tool-use surface shipped as a built-in consumer feature, predating the general MCP-like surface of stage 6 by ~18 months at OpenAI. Stage 7 (deep research) draws on both stages 3 and 5 — at every lab where it shipped as a product, deep research arrived within 1–5 months of having both browsing and reasoning in place. DeepSeek is the negative case that confirms the framework-not-model framing: it ships stages 3 and 5 (Internet Search in V2.5; R1 reasoning) and the model components that would enter a deep-research loop (V3.1's search-agent claims, V3.2's thinking-in-tools), but has not packaged the composer as a public product within the cutoff.
...
![*Figure 5. Empirical capability ordering across N=5 labs. (a) Transition-order matrix: each cell shows the fraction of labs where stage i (row) shipped before stage j (column), conditional on both being present; N per cell. Green cells (near 100%) indicate consistent ordering; yellow cells (near 50%) indicate parallel emergence; red cells (near 0%) indicate reverse ordering. (b) Time gap in months for transitions with N ≥ 4. Negative gaps mean j shipped before i at that lab. The matrix confirms that stages 3 and 4 are empirically parallel (browsing and code execution, N=3 labs with both), stage 5 (reasoning) follows stages 1–3 consistently, and stage 7 (deep research) follows stages 3 and 5. Descriptive historical record, N=5 labs — no statistical inference.*](inputs/generated/fig_capability_dag.pdf)
