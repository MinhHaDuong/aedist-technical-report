...
![*Figure 5. Empirical capability transition matrix across N=5 labs. Each cell shows the fraction of labs where stage i (row) shipped before stage j (column), conditional on both being present; N per cell. White = no lab made that transition; dark green = all labs did. Stages 3 and 4 (browsing × code execution) show a three-way split — genuinely parallel. Stage 5 (reasoning) follows stages 1–3 consistently. Stage 7 (deep research) draws on stages 3 and 5. Descriptive historical record, N=5 labs — no statistical inference.*](inputs/generated/fig_capability_dag.pdf)
