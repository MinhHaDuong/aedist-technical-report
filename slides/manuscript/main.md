...
This paper addresses a concrete task — building a complete, sourced inventory of Vietnam's thermal power plants — as a lens for evaluating whether current AI systems can produce research-grade statistical data. We situate the work against the empirical landscape of open power-plant databases (§2), define a four-dimensional quality bar (§3), survey the AI capability landscape (§4), establish a parametric ceiling with 14 language models (§5), test the state-of-the-art commercial frontier (§6), and show that targeted pipeline design is needed to bridge the gap (§7). The Discussion (§8) examines the limits of F1 as an aggregate and prefigures a fusion-based research programme.
...
**Why we re-compile rather than reuse.** Global Energy Monitor is the closest external inventory to ours, and one might ask why the reference is built afresh rather than adopted from GEM. Four reasons. *Per-cell provenance*: the research-grade bar (§3) demands that each value (capacity, status, commissioning date) trace to a specific primary source, whereas aggregate trackers carry source attribution at the record level at best. *Under-documented sites*: GEM and WRI cover documented, large-capacity units well; the forward-looking PDP8 pipeline of proposed and planned LNG and gas projects, much of it in Vietnamese-language master-plan annexes, is exactly the under-documented tail they thin out. *Reproducible compilation*: a single-author manual compilation from primary sources (PDP7/7A/8 annexes, EVN reports, MOIT decisions), version-locked and accompanied by per-row source priority, is auditable end-to-end in a way a redistributed aggregate is not. *Licence*: the reference and its provenance ship under CC BY 4.0, free of the redistribution constraints that encumber several upstream sources. The long-tail figure makes the documented-head / under-documented-tail split concrete (Figure 1): the 177 reference plants, sorted by visibility, are checked against four recognition layers, and the speculative tail thins out from layer to layer.

![](../report/inputs/generated/fig_longtail_recognition.pdf)\

*Figure 1. Recognition long tail across the 177 reference plants (Gold layer, by construction), sorted by descending visibility. Each plant is checked against four layers: the Gold reference (all 177), the Global Energy Monitor tracker (157), Wikipedia coal-list and gas-section coverage (130), and the parametric model census (144 plants recognised by at least one Experiment 1 run). The documented head is covered by every layer; the speculative tail — proposed and planned PDP8 pipeline — thins out layer by layer, and is the reference's unique contribution. Counts re-derived from `data/reference/tab_longtail_layers.csv`.*
...
## 3. Quality dimensions for research-grade statistical datasets
...
**Cohorts.** The sweep dispatched 16 models (the `modelset_exp1_journal` set in `experiments/experiments.toml`). Two — `qwen3.6-27b` and `qwen3.6-plus` — were dropped before analysis, leaving the 14-model analysis cohort (`modelset_exp1_batch2`) that backs every per-model number, Figure 3, Figure 4, and Figure 5. The family-panel spider (Figure S1, Annex D) renders 12 of these 14: it lays out one panel per architectural family (Claude, GPT, Mistral, Qwen) and so omits the two DeepSeek models, which have no family panel. Every figure caption below names the cohort it draws on.
...
*Figure 4. Plants correctly identified vs cost per call across the Experiment 1 lineup, split by architectural family across two panels with shared axes: **panel (a)** Claude, GPT, Mistral; **panel (b)** Qwen, DeepSeek. Each of the fourteen `modelset_exp1_batch2` models contributes a filled square at its pooled median true-positive count and a marker at every individual rep, plotted at the rep's own per-call cost (cents USD, log scale); the dashed line at 177 marks the full Vietnam thermal inventory. Marker shapes, colour palette, the per-rep cohort split, and the audit artifact (`report/inputs/generated/cost_quality.csv`) are detailed in Annex B.*

![](../report/inputs/generated/fig_quality_floor_heatmap_exp1.pdf)\

*Figure 5. Quality-floor heatmap for Experiment 1 (parametric arm, 14 models × 5 runs). Columns are the same fourteen models as Figure 3, laid out in the same order — architectural family (Claude, DeepSeek, GPT, Mistral, Qwen) then size descending (larger first) — with DeepSeek included. Rows are the sub-score criteria, grouped by dimension and derived programmatically from the scoring CSV. Only **discriminating** sub-scores are shown: a criterion every model clears uniformly (the all-green rows — capacity sanity, core/capacity completeness, source presence, dual-source, as-of presence, each ≈1.0 across the board, with the whole Field-completeness dimension thus omitted) carries no quality-floor signal and is dropped automatically when its across-model spread falls below 0.1. The surviving rows span Accuracy, Coherence, Provenance, and Temporality. Each cell is the **per-model mean** of that sub-score across the five runs, rendered on a continuous red→green scale (0 = every run fails the criterion, 1 = every run clears it); the simple mean suffices because a model that systematically zeros a free criterion averages to ≈0 and reads dark red, the §3 "too weak to clear the bar" signal. The §5 internal-coherence screen (the reference-free ρ=0.92 veto, `cap_distinct ≤ 4 OR status_distinct ≤ 1`) is **merged into the Coherence dimension** as its positive complement (`1 − veto`, the "Internal coherence" row), so higher is better in lockstep with every other sub-score rather than occupying a separate disqualifying column. The §3 quality bar is a conjunction: a near-zero (dark red) cell anywhere in a model's column means the bar is not cleared. Model labels are family-coloured and dimension-separation lines isolate the criterion groups.*

## 6. Experiment 2: the commercially available frontier still falls short

How well do state-of-the-art tools perform at producing research-grade statistical datasets? The parametric ceiling of §5 is a deliberately handicapped baseline — no web, no tools, no reasoning budget beyond what each model carries internally. The commercially available frontier, by contrast, ships agents that combine extended reasoning, web search, document ingestion, and tool use into a single "deep research" surface. The question is whether removing the §5 handicaps suffices to clear the §3 quality bar. Even the best independent public tracker, Global Energy Monitor, covers only 89% of the reference after light review (Annex B); the frontier agents are therefore measured against an inventory no single open database fully reproduces.

The quality dimensions §6 measures are the *operational and accuracy* ones: row-level F1 against the reference, coverage (plants enumerated), and per-row provenance counts (Source 1 / Source 2 citation presence, Annex D). The §3 dimensions that require *peer cross-evaluation* — coherence, provenance support, and temporality scored on a rubric by judging agents — are deferred to Phase C (cross-model judging), reserved for post-conference analysis; §6's claims rest on the F1, coverage, and citation-count metrics, not on the deferred cross-eval scores.

**Experiment 2 — SOTA frontier (Annex C).** We conduct an experiment with four state-of-the-art cloud AI agents that have extended reasoning and web access, queried over direct vendor APIs (no browser automation): Anthropic Claude Opus 4.6 (US, web_search + adaptive thinking), OpenAI GPT-5.5 (US, Responses API + web_search + reasoning), Mistral Large 2512 (FR, Agents API + web_search connector), and Qwen3-Max via DashScope (CN, web_search inside thinking mode). The fourth slot is hypothesis-relevant rather than decorative: Chinese-language investor and trade documents on Vietnamese power assets are under-indexed by Western search. The experiment runs two arms over the same four agents (N=5 each). **Arm 1 (naive)** — a single-shot prompt (Doc-07) with no scaffolding, web on; the null comparator. **Arm 2 (optimised, simple-harness multi-turn)** — a multi-turn protocol in which each agent first designs its own prompt and settings (Phase A), runs once as a smoke gate (Phase B-0), then runs N=5 against a single provider under a per-session cap of 50K tokens / $3 (Phase B); this constitutes a simple harness — an LLM orchestrator (DeepSeek classifier) controlling a loop with the tested LLM as the tool. The naive-vs-optimised contrast isolates the protocol's contribution over raw model capability. Row-level F1 against the 177-plant reference is now scored for all four arms (Table 1); cross-model judging on the four §3 dimensions remains reserved for post-conference analysis (Phase C).
...
- **T1 — Snapshot currency**: each value carries a source-publication date and a best-effort "as-of" validity period; status changes since that date are flagged. This is what §3 requires and what the experiments measure.
...
The present paper scopes to T1. T2 is acknowledged as the stronger requirement for IAM and scenario-projection use cases; it is not in scope here. A GEM-style event log may become necessary when implementing the verified-provenance layer (§7, future work) — each verification act is itself a timestamped event — but that design choice is deferred to a subsequent paper.

The narrative inventory component of §7 handles longitudinal aspects pragmatically: plant histories, PDP-cycle reclassifications, and developer-name changes appear as free-text annotations rather than as structured event records. This is a deliberate scoping decision, not an architectural commitment.
...
Anthropic is pinned to Opus 4.6 (not 4.7) per the 2026-05-20 compose decision: identical call shape, ~40% lower per-call cost on the verified probe. Kimi K2.6 is disqualified — vendor docs document `thinking` and `$web_search` as mutually exclusive, fatal for §6's "extended reasoning AND web access" requirement. Geographic / corpus spread (US × 2 + FR + CN) is hypothesis-relevant: Chinese-language investor and trade documents on Vietnamese power assets are under-indexed by Western search.

### Baseline reference for cross-comparison

The naive one-shot baseline that §6 compares against is **not** §5's `modelset_exp1_journal` sweep. It is the pre-existing `experiments/outputs/direct_complete/` artifact from `sweep_direct_complete` over `modelset_frontier_10labs` — a wider lab survey at a single rep per model. Phase C will read from it directly; it is not re-run.

### Phase structure

**Phase A — Reflexive prompt design (optimised arm).** Each agent receives the baseline prompt (`experiments/prompts/prompt_complete.txt`), the four §3 quality-dimension paragraphs verbatim, the task statement, a JSON envelope spec, and the per-session budget cap (announced upfront — see "Phase B dialogue policy" below). The agent is asked to return a fully-specified design — `system_prompt` (threaded into the provider's system field at agent creation), `designed_prompt` (the user-side prompt sent on turn 1 of Phase B), `settings` (`thinking`, `max_tokens`), and `rationale` — that maximises the quality of the report it will produce within the dual-axis session cap (below) and an overnight wall-clock. Outputs land as `<agent>_phase_a.json` with the four envelope fields.
...
#### Phase B dialogue policy (fixed experimental condition)
...
- **A documents axis was added.** Beyond the two registered arms, two further *unregistered, exploratory* conditions — the same naive and optimised surfaces with a reference document pack attached — were run, expanding the corpus from 40 to 80 sessions and forming the 2×2 factorial (query mode × documents) of §6's Figure 6. These two conditions are exploratory by construction: they are never aggregated with the two registered arms.
...
- **Phase C cross-evaluation is deferred.** The four-dimension peer cross-evaluation was reserved for post-conference analysis; the as-run results below and in §6 are the operational and F1-based metrics, not the §3-dimension cross-eval scores.
...
Phase B outputs are evaluated against the 177-plant reference using the same `src/aedist/evaluate.py` machinery as §5 (LP matcher, ADR-2/3, `similarity_threshold = 90`, `capacity_weight = 0.001`), yielding row-level F1 and per-attribute accuracies. Phase C cross-evaluation — peer scoring of each output on the four §3 dimensions — is deferred to post-conference analysis; the as-run results are the F1-based and operational metrics of §6, not the §3-dimension cross-eval scores.

### What this experiment does and does not test

This experiment tests whether removing the §5 handicaps (no web, no tools, no documents), by handing the task to a commercially available deep-research surface, clears the §3 quality bar. It does not test custom workflows or local models; that is §7. It does not test how the four dimensions trade off under a fixed budget; that is §7 once the composite-quality scorers exist. It does not test browser-automated surfaces (ChatGPT.com, Claude.ai chat UI) — only direct vendor APIs.
...
*Figure S1. Experiment 1 quality profiles by model family in a 2×2 spider layout (12 of the 14 analysis-cohort models): one panel each for Claude, GPT, Mistral, and Qwen. The two DeepSeek models are omitted — DeepSeek has no family panel in this layout — so this figure covers 12 models, not the full 14. Each trace is a model median over five runs; the shaded ring marks min-max spread across runs. Accuracy-related axes remain elevated while provenance and temporality stay near-zero, making the quality-bar gap visible despite strong parametric recall on selected models.*
...
Figure S4 places each of the 177 reference plants on its own row (names read left-to-right, no rotation) against the 70 Experiment 1 runs (14 models × 5 repetitions) as columns. A blue cell marks a recognised plant (true positive); an empty cell, a miss. Rows are grouped by status, then by decreasing capacity, using the same status categories as the difficulty table below. To keep plant names legible at print scale, the matrix is split across pages by lifecycle stage. A final page lists the 40 most frequent false positives across all runs, sorted by decreasing occurrence, in red — the paper's false-positive convention (unmatched: extracted but not in the reference; possibly real, not necessarily fabricated). Unlike Figure 3, which only counts each run's recognised plants, the fixed-row alignment reveals *which* plants each model misses: famous plants form dense rows, obscure ones sparse rows.

*Figure S4. (Next three pages.) Experiment 1 recognition matrix, transposed to portrait so plant names stay legible: the 177 reference plants are rows (grouped by status, then by decreasing capacity, with the status bands labelled in the right margin) and the 70 runs (14 models × 5 repetitions) are columns under a three-level family / version / repetition header, in Figure 3's order: family, then decreasing effective parameter count. Blue = plant recognised. Pages 1–2 split the plants by lifecycle stage (operational, retired, and cancelled plants, then proposed, planned, and under-construction ones); page 3 lists the 40 most frequent false positives (red).*
...
The §8 Discussion paragraph "Internal coherence as a zero-reference screen" reports a pooled Spearman ρ = 0.92 between within-run capacity variability and reference-based F1 across 70 Experiment 1 runs. A potential confound: weak models are weak across the board, so the correlation may reflect *model quality* rather than run quality — the screen may be near-tautological at the model grain. This annex tests whether the screen discriminates good from bad runs of the **same model**, removing the across-model confound.
...
## Annex G — Capability rollout: per-lab shipping order

This annex records the detailed shipping-order observations behind Figure 2, kept out of the §4 body to keep the capability discussion at the level of the envelope framing. The figure is descriptive: no claim is made that any lab is "ahead" or that the feature order is forced.
