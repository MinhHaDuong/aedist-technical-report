...
**Observed results (provenance quality).** Beyond row counts, every Source 1, Source 2, and Notes cell across all 40 runs was parsed to measure citation coverage and corroboration (full methodology and table in the technical analysis report, `docs/exp2-analysis-v0.md` §3.5). Figure 6 plots each run as inventory rows found (coverage) against rows where a second source was also cited (corroboration). The diagonal marks full double-sourcing. OpenAI optimised is the only agent to achieve both high coverage (100–180 rows) and substantial corroboration (70–98 rows with Source 2), maintaining roughly half the diagonal rate even at the highest row counts. Claude optimised reaches similar coverage but hits a corroboration ceiling of 40–65 rows regardless of how many plants it finds — the additional rows unlocked by the multi-turn protocol are largely single-sourced. Qwen clusters near the diagonal at low coverage (10–20 rows), meaning nearly every plant it enumerates is double-sourced but the inventory remains small. Several Claude and Mistral naive runs sit at zero corroboration despite finding 50–75 rows; the optimised protocol eliminates this failure mode for both agents. To our knowledge, per-cell provenance analysis of this kind has not been published for frontier deep-research agents.

![Figure 6](inputs/generated/fig_exp2_coverage_certainty.pdf)

*Figure 6. Coverage vs. corroboration across 35 Exp2 runs (5 with zero parsed inventory rows excluded). X-axis: inventory rows enumerated. Y-axis: rows with a Source 2 citation present. The dashed diagonal marks full double-sourcing (every row corroborated). Filled circles: optimised arm; open circles: naive arm. Colours encode agents. OpenAI optimised alone sustains high values on both axes; Claude optimised shows a corroboration ceiling despite growing coverage; Qwen clusters near the diagonal at low volume.*
