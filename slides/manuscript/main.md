...
The quality dimensions §5 measures are the *operational and accuracy* ones: row-level F1 against the reference, coverage (plants enumerated), and per-row provenance counts (Source 1 / Source 2 citation presence, Annex D). The §2 dimensions that require *peer cross-evaluation* — coherence, provenance support, and temporality scored on a rubric by judging agents — are deferred to Phase C (cross-model judging, ticket 0171), reserved for post-conference analysis; §5's claims rest on the F1, coverage, and citation-count metrics, not on the deferred cross-eval scores.
...
- **A documents axis was added.** Beyond the two registered arms, two further *unregistered, exploratory* conditions — the same naive and optimised surfaces with a reference document pack attached — were run, expanding the corpus from 40 to 80 sessions and forming the 2×2 factorial (query mode × documents) of §5's Figure 5. These two conditions are exploratory by construction: they are never aggregated with the two registered arms.
...
Figure 7 places each of the 177 reference plants on its own row — names read left-to-right, no rotation — against the 70 Experiment 1 runs (14 models × 5 repetitions) as columns. A blue cell marks a recognized plant (true positive); an empty cell, a miss. Rows are grouped by status, then by decreasing capacity, using the same status categories as the difficulty table below. To keep plant names legible at print scale, the matrix is split across pages by lifecycle stage. A final page lists the 40 most frequent false positives across all runs, sorted by decreasing occurrence, in red — the paper's false-positive convention (unrecognized: possibly real, not necessarily fabricated). Unlike Figure 2, which only counts each run's recognized plants, the fixed-row alignment reveals *which* plants each model misses: famous plants form dense rows, obscure ones sparse rows.

*Figure 7. (Next three pages.) Experiment 1 recognition matrix, transposed to portrait so plant names stay legible: the 177 reference plants are rows (grouped by status, then by decreasing capacity, with the status bands labelled in the right margin) and the 70 runs (14 models × 5 repetitions) are columns under a three-level family / version / repetition header, in Figure 2's order: family, then decreasing effective parameter count. Blue = plant recognized. Pages 1–2 split the plants by lifecycle stage (operational, retired, and cancelled plants, then proposed, planned, and under-construction ones); page 3 lists the 40 most frequent false positives (red).*
