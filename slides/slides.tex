...
\subtitle{A pilot study to de-risk an extraction benchmark}
...
\textbf{Energy transition tracking requires plant-level data.}

\medskip
In countries with weak statistical offices, expert teams manually compile power plant inventories from government plans, utility reports, and industry databases.

\medskip
This is slow, expensive, and doesn't scale.

\medskip
\textbf{Can AI produce these tables automatically?}

\medskip
The obvious approach: ask a large language model.
But \emph{which model}? And does the model even matter,
compared to \emph{how} you ask?

\medskip
{\small This talk presents a \textbf{pilot study} on one task
to validate the design of a reusable benchmark instrument.}
...
\medskip
\centering\small
These are not flaws --- they are \textbf{design requirements} for the full instrument.
\end{frame}

% -------------------------------------------------------------------
\begin{frame}{From pilot to instrument}
\begin{columns}[T]
\column{0.55\textwidth}
\textbf{Requirements derived from the pilot:}

\medskip
\begin{tabular}{@{}lp{4.2cm}@{}}
\toprule
\textbf{Pilot gap} & \textbf{Instrument spec} \\
\midrule
1 task, 1 country     & Task battery: 4+ countries
                        and sectors \\[3pt]
Memorization risk     & Synthetic reference
                        (adversarial control) \\[3pt]
Flat method enum      & Composable pipeline:
                        knowledge $\times$ conversation
                        $\times$ decomposition
                        $\times$ verification \\[3pt]
No uncertainty        & Bootstrap CIs,
                        paired significance tests \\[3pt]
Matching confound     & Sensitivity sweep
                        on fuzzy threshold \\
\bottomrule
\end{tabular}

\column{0.42\textwidth}
\textbf{What the pilot validated:}
\begin{enumerate}
  \item Schema works (Plant, RunRecord, reconciliation)
  \item Evaluation pipeline is reproducible and
        resume-safe
  \item Manager/worker dispatch scales to 37 models
  \item Cost tracking is precise (\$0.82 auditable)
  \item measurements.jsonl as single source of truth
\end{enumerate}

\medskip
\textbf{The pilot de-risks the instrument.}\\
{\small The pipeline exists. The question is whether the
findings generalize.}
\end{columns}
\end{frame}
