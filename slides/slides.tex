...
Vietnam thermal fleet $\to$ PyPSA input data.
\end{columns}
\end{frame}
...
\medskip
\textbf{1.\ This benchmark} --- accuracy proved\\[2pt]
{\small Vietnam thermal: 163 plants, 35 models $\times$ 6 methods.\\
RAG + decomposition: 99\% recovery, \$0.06.\\
Method validated on \emph{one} component class.}

\medskip
\textbf{2.\ Trustworthy output} --- provenance next\\[2pt]
{\small Every datum traceable to a source PDF.\\
Confidence intervals. ``I don't know'' when uncertain.\\
The gate to usability by modelers.}

\medskip
\textbf{3.\ Complete PyPSA-ASEAN input} --- then scale\\[2pt]
{\small All components: thermal, RE, storage, transmission.\\
10 ASEAN countries, same method, different corpus.\\
${\sim}\$4$ for a complete regional power system database.}

\column{0.42\textwidth}
\textbf{Four-level evaluation:}

\medskip
\begin{tabular}{@{}clp{2.8cm}@{}}
\toprule
 & \textbf{Level} & \textbf{Maps to} \\
\midrule
\checkmark & \textbf{1.} Resources &
  Measured \\[3pt]
\checkmark & \textbf{2.} Quality &
  This pilot \\[3pt]
 & \textbf{3.} Provenance &
  Trustworthy output \\[3pt]
 & \textbf{4.} Usability &
  ASEAN, all components \\
\bottomrule
\end{tabular}

\medskip
{\small Accuracy is proved.\\
Trust is next.\\
Scale follows.}
\end{columns}
\end{frame}
