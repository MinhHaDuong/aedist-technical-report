...
% -------------------------------------------------------------------
\begin{frame}[fragile]{\NumModelsTested{} modèles, 163 centrales~: le modèle seul ne suffit pas}
...
\bottomrule
\end{tabular}
% REVIEW MINOR: van Fraassen / Popper attributions are rhetorical stretches.
% A referee will note that van Fraassen's empirical adequacy ≠ RAG citation,
% and Popper's falsifiability ≠ per-cell citations. Consider weakening to
% "inspiré de" or dropping the footnotes and keeping only the criteria names.
...
% REVIEW MINOR: Watcher → Fraîcheur is debatable; discovery enables freshness
% only when paired with the Analyst's incremental fusion. Could map Watcher
% to Ancrage (finds new sources) and Analyst to Fraîcheur (does the update).
...
\bigskip
\begin{center}
% REVIEW MINOR: "trackés" and "de-risque" are English loanwords likely to draw
% comment at a French-language venue. Alternatives: "suivis" (trackés),
% "réduit les risques de" or "valide" (de-risque). Also "benchmark" below.
% REVIEW MINOR: No explicit bridge from this v1 vision to the running v0
% prototype. Consider adding: "v0 est le prototype en cours ; ce benchmark
% en valide les choix de conception."
...
\begin{description}
  ...
  \item[Échelle] \NumModelsTested{} modèles, \$0.82 au total
  \item[Conditions clés] Documentation $\times$ Conversation $\times$
        Décomposition $\times$ Agrégation $\times$ Modèle (\NumModelsTested{})
\end{description}

\medskip
{\small \textbf{Métrique clé~:} centrales trouvées sur 163.\\
La couverture varie de 4 à 163~; la précision varie de~27\% à~100\%.\\
Le problème est d'abord la \textbf{couverture}, mais les hallucinations existent (points rouges).}
\end{frame}
...
\toprule
% REVIEW MINOR: STANAG 2022 reference in this header is unexplained and
% not developed elsewhere on the slide. Either add a footnote defining the
% NATO credibility scale (A–F × 1–6) or drop the parenthetical entirely.
...
% REVIEW MINOR: "trackable" is an English loanword. Alternative: "versionnable".
...
\end{center}
\smallskip
% REVIEW MINOR: "benchmark" used as a French verb. Alternative: "mesure" or
% "évalue". Second "benchmark" (as noun) is acceptable but could be "évaluation".
