...
\textbf{Setup:} Best extraction (DeepSeek V3.2, decomposed, 167 plants, F1=98.8\%).\\
Verify each plant against reference database. Filter by evidence score threshold.
...
$\geq 3$ (one primary source) & 102 & \textbf{100\%} & 62.6\% & 77.0\% \\
\bottomrule
\end{tabular}
\end{center}

\medskip
\textbf{Insight:} Requiring one primary source eliminates \textbf{all 4 hallucinations}\\
but drops 65 real plants --- the \emph{reference} is incomplete, not the extraction.

\medskip
\textbf{Implication:} The pipeline finds more plants than the reference can confirm.\\
Verification measures reference completeness as much as extraction quality.
\end{frame}
