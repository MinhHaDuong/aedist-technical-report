...
\textbf{5 models} $\times$ \textbf{5 methods} $\times$ 3 runs each.
Each dot = 1 power plant.
...
\begin{center}
\begin{tikzpicture}
\pgfplotstableread[col sep=comma]{inputs/generated/method_convergence.csv}\convdata
\pgfplotstablegetrowsof{\convdata}
\pgfmathtruncatemacro{\lastrow}{\pgfplotsretval-1}
%
% Method band layout: each method gets a Y band.
% Runs sorted by TP descending within each band.
% Blue circles for TP (x=1..tp), orange for FP (x=-1..-fp, clipped to 50).
%
\begin{axis}[
    width=0.92\textwidth,
    height=6.5cm,
    ...
    xmin=-55, xmax=175,
    ymin=-1, ymax=32,
    ytick={2.45, 9.10, 15.85, 22.55, 29.30},
    yticklabels={Single, Multi-turn, Web, RAG, Decomposed},
    y tick label style={font=\small},
    extra x ticks={163},
    extra x tick labels={163},
    extra x tick style={grid=major, grid style={dashed, green!60!black}},
    scatter, only marks,
    scatter src=explicit,
    scatter/classes={
        tp={mark=*, mark size=0.3pt, matched},
        fp={mark=*, mark size=0.3pt, halluc}
    },
    clip=false,
]
\addplot[scatter] table [x=x, y=y, meta=type, col sep=comma]
    {inputs/generated/method_dots.csv};
% Zero line
\draw[black!30, thin] (axis cs:0,\pgfkeysvalueof{/pgfplots/ymin})
...
\smallskip
{\small \textbf{Decomposed} finds all 163 plants but hallucinates.
\textbf{Web} never hallucinates but caps at $\sim$50.
No method yet converges all models to 163 with zero errors.}
\end{frame}
...
\begin{itemize}
  ...
  \item \textbf{Best overall: 163/163, F1=98.8\%, \$0.06}
\end{itemize}

\medskip
\textbf{Bottom line:}\\
An off-the-shelf pipeline with the right \textbf{method conditions} recovers 98.8\% of an expert inventory for \$0.06.
\end{columns}
\end{frame}
...
\begin{itemize}
  ...
  \item Off-the-shelf pipeline recovers 98.8\% of an expert
        inventory for \$0.06
...
\begin{enumerate}
  \item The right methods recover 98.8\% of an expert
