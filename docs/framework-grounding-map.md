...
| **Method quality — verifiable** (axis) | method | Evidence score 1-3; Falsifiability (QG §2.2) | operationalises | A citation makes a claim falsifiable (Popper). Evidence score 1-3 classifies whether a source is offered, secondary, or primary. One agent with the right prompt can reach this floor. Score 0 (Fabricated — source checked and found false) is excluded from both tiers: a fabricated claim fails the method axis entirely rather than mapping to a framework concept. |
