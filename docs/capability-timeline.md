...
chronology of capability stages (chat LLM -> web search -> code
execution -> files upload -> reasoning -> external tool use -> deep
research -> autonomous agents) at the five labs covered by Experiment 1's panel:
...
- **Chat LLM comes first** — every lab ships a consumer chat product
  before any tool or reasoning surface.
- **Web search and reasoning emerged as parallel branches** — browsing
  and reasoning model surfaces both shipped as products in 2024, with
  per-lab ordering inversions (Anthropic shipped reasoning before web
  search).
- **Deep research is the join of web search and reasoning** — multi-step
  web retrieval driven by a reasoning model, with citations.
...
- **OpenAI's file-upload boundary is fuzzy.** ChatGPT plugins (March
  ...
  uploads, no plugin needed) as the files-upload milestone because that is
...
- **Alibaba (Qwen) has the broadest agentic developer surface but
  ...
  in front of consumers. External tool use (MCP) arrived
  ...
  has been corrected. Web search is dated to the
  ...
  remaining absences are deliberate: code execution (as a
  ...
  sandbox are developer-facing surfaces. Autonomous agents (multi-agent
  consumer product) are absent because Qwen-Agent remains a developer
  framework — there is no QwenChat-side multi-agent product.
  Deep research (on Qwen Chat, 2025-05-13) is the only cell
...
- **DeepSeek has the most compressed stack of any focus lab.** All
  four productised capabilities (chat, web search, files upload,
  reasoning) ship between Nov 2023
  (DeepSeek LLM 67B + chat.deepseek.com) and Jan 2025 (DeepSeek
  App + R1) — 14 months, faster than Mistral's 17-month run.
  Chat-LLM note: the 2023-11-29 GitHub repo release linked
  ...
  composition. Deep research, code execution, external tool use,
  and autonomous agents are absent:
  ...
  cutoff. The prior external-tool-use CSV entry (2024-07-25 Function Calling
...
context. Chat LLM: 2023-03-21 Bard [(blog)][14] / 2023-12-06 Gemini 1.0
[(blog)][16]. Web search: search-grounded since Bard 2023; 2024-12-11
Gemini 2.0 native tool use [(blog)][17]. Code execution: part of the
2024-12-11 Gemini 2.0 native tool use launch [(blog)][17] (Gemini app
code-execution tool). Files upload: 2023-07-12 NotebookLM [(blog)][15].
Reasoning: 2024-12 Gemini 2.0 Flash Thinking Experimental [(blog)][18].
External tool use: 2024-12-11 Project Mariner [(blog)][17] / 2025-10
Gemini 2.5 Computer Use [(blog)][20]. Deep research: 2024-12-11 Gemini
Deep Research [(blog)][19]. Autonomous agents: 2025-05-20 Project
Mariner multi-task agent system (I/O 2025) [(blog)][27].
...
[6]: https://openai.com/index/function-calling-and-other-api-updates/ "Function calling and other API updates (OpenAI, 2023-06-13). Note: files-upload milestone for OpenAI is the 2023-10-30 ChatGPT All-Tools rollout (PDF / file analysis without plugin); covered in ChatGPT release notes, no standalone blog post."
...
[15]: https://blog.google/technology/ai/notebooklm-google-ai/ "NotebookLM (Google, 2023-07-12). Source-grounded retrieval over user-uploaded documents — Google's files-upload product."
...
[33]: https://github.com/deepseek-ai/DeepSeek-LLM "DeepSeek LLM GitHub repository (deepseek-ai, first commit 2023-11-29T11:06Z). Open-weights release of 7B/67B base and chat models; original README (commit f8b3d77) links chat.deepseek.com — confirming the consumer chat product was live from launch day. Chat-LLM milestone for DeepSeek. The api-docs.deepseek.com/news/news1129 URL (the original citation) returns 404."
...
[36]: https://api-docs.deepseek.com/updates "DeepSeek Function Calling API (DeepSeek, 2024-07-25). API-level capability; not a consumer product. External tool use for DeepSeek is absent: chat.deepseek.com has no user-facing agentic tool surface as of 2026-05-22 cutoff."
[40]: https://alternativeto.net/news/2025/7/qwen-chat-launches-desktop-app-with-mcp-support-and-multitasking-features/ "Qwen Chat Desktop (macOS) launches with MCP support (AlternativeTo, 2025-07-14, reporting on lab X post). First consumer-facing agentic tool surface for Alibaba / Qwen: end-users can configure and invoke MCP servers from the desktop app. Windows version followed 2025-08-15 (x.com/Alibaba_Qwen/status/1956399490698735950). External-tool-use milestone for Alibaba."
[37]: https://api-docs.deepseek.com/news/news250120 "DeepSeek-R1 (DeepSeek, 2025-01-20)."
[38]: https://help.openai.com/en/articles/6825453-chatgpt-release-notes "ChatGPT release notes (OpenAI, 2023-07-06 entry). Code Interpreter beta rollout to ChatGPT Plus users on web; alpha was available via plugins waitlist 2023-03-23 (openai.com/index/chatgpt-plugins/). Code-execution milestone for OpenAI."
[39]: https://claude.com/blog/analysis-tool "Introducing the analysis tool in Claude.ai (Anthropic, 2024-10-24; originally anthropic.com/news/analysis-tool, 308 redirect). JavaScript code sandbox in Claude.ai. Update note from 2025-11-05 records that the analysis tool was replaced by more powerful code execution capabilities. Code-execution milestone for Anthropic."
[41]: https://mistral.ai/news/le-chat-mistral "Le Chat beta (Mistral, 2024-02-26). First public consumer chat product from Mistral. Mistral 7B Instruct (2023-09-27) was an open-weights model release without a consumer chat interface; Le Chat beta is the chat-LLM milestone."
[42]: https://x.com/Alibaba_Qwen/status/1882818582758961351 "Qwen Chat v0.2 launch with built-in web search (Alibaba_Qwen on X, snowflake-decoded 2025-01-24T15:51 UTC). Primary source from the lab handle, same format the matrix accepts for Alibaba deep research ([31]). Cross-confirmed by the 2025-01-27 qwen2.5-1m blog referencing Qwen Chat as supporting 'perform searches'. Web-search milestone for Alibaba; supersedes the absent decision from ticket 0232 / PR #426 which only considered the 2025-03-07 Qwen-Agent QwQ-32B demo."
